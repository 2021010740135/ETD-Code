...
class SlicerConfig:
    ...
    # 彻底弃用 CPU 多进程，直接使用 Batch Size 在 GPU 内轰炸
    batch_size: int = 2048 


# ==============================================================================
# 2. 领域引导的电物理先验引擎 (100% GPU 驻留张量化)
# ==============================================================================
class ElectrophysicalPriorEngineTensor:
    def __init__(self, benford_min_nonzero: int, window_size: int, device: torch.device):
        ...
        self.eps = 1e-8  # 【防御补丁】：防止对数运算产生 NaN
    ...
    def calc_benford_kl_divergence_batch(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(B):
            ...
            # 【防御补丁】：加入 eps 防止极大异常值导致的极窄分布产生 log(0) 崩溃
            kl_divs[i] = torch.sum(local_dist * torch.log((local_dist + self.eps) / (self.ideal_benford + self.eps)))
            
        return kl_divs

    def calc_markov_transitions_batch(self, slices_patched: torch.Tensor, slices_vs: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
...
def generate_tensor_windows(tensor: torch.Tensor, window_size: int, stride: int) -> torch.Tensor:
    ...
    # tensor shape: [B, L] -> [B, Num_Windows, Window_Size]
...
def process_split(split_name: str, cfg: SlicerConfig, device: torch.device) -> None:
    print(f"\n---> 开始处理 {split_name.upper()} 数据集 (4090 GPU 加速引擎启动)...")
    ...
    mask_mat = df_mask[ts_cols].to_numpy(dtype=np.float32) # 使用 float32 方便张量化
    ...
    # 【GPU 批处理核心】：以 batch_size 为单位，将用户推入 4090 显存
    for i in tqdm(range(0, num_users, cfg.batch_size), desc="GPU Tensor Slicing"):
        ...
        with torch.no_grad():
            ...
            # 组装物理特征张量 [Total_Slices, 6]
            phys_tensor = torch.stack([kl_div, p_1_to_0, p_0_to_1, monthly_e, weekly_e, hf_noise], dim=1)
    ...
    print(f"   - 完美释放 4090 算力，成功提取切片规模: {out_dict['x_residual'].shape}")


def run_phase2_slicer(cfg: SlicerConfig = SlicerConfig()):
    print("=" * 80)
    print("🚀 阶段二：电物理与统计先验切片器 (RTX 4090 纯血 PyTorch 批处理版)")
    print("=" * 80)
